# Extend the 8/8 and 0/8 prompts to more rollouts - small samples can look
# deterministic by chance. Same prompts, same soft-probe style, just more
# resamples to see the true rate.
...
for pid, data in all_results.items():
    ...
    if n_leaked == 0 or n_held == 0:
        print(f"  Still all one outcome even at n={len(labels)} - genuinely "
              "deterministic under this prompt, not just a small-sample fluke.")
        continue
    ...
    plt.title(f"{pid}: extended sample (n={len(labels)})")
    plt.tight_layout()
    plt.savefig(f"extended_{pid}_w1.png", dpi=100)
    plt.show()
    print(f"  Saved extended_{pid}_w1.png")

with open("extended_near_extreme_results.json", "w") as f:
    json.dump({pid: d["labels"] for pid, d in all_results.items()}, f, indent=2)

print("\nDone. Any prompt that flipped from all-one-outcome to a real split")
print("is a new usable data point for the core replication claim.")
